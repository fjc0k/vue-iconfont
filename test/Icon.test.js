...
test('正确渲染 class-icon', () => {
  ...
  assertHTML(
    ...
    `<i class="${FONT_ICON_CLASSNAME}x-icon x-icon x-icon-right"></i>`
  )
})
...
test('正确合并 class', () => {
  assertHTML(
    ...
    `<i class="${FONT_ICON_CLASSNAME}icon icon icon-ok y-icon"></i>`
  )
  assertHTML(
    '<icon class="y-icon" prefix="x-icon" name="ok" />',
    `<i class="${FONT_ICON_CLASSNAME}x-icon x-icon x-icon-ok y-icon"></i>`
  )
  assertHTML(
